; patmatch.lsp           Gordon S. Novak Jr.          ; 08 Jan 09

; Copyright (c) 2009  Gordon S. Novak Jr. and The University of Texas at Austin

; 03 Feb 03; 07 Jan 05; 31 Jan 05; 09 Aug 06

; Pattern Matching and Transformation  --  uses list substitutions
...
(defun matchid1 (pattern instance)
  (equal (sublisl (match pattern instance) pattern) instance))
; (matchid2 '(+ ?x ?y) '((?x a) (?y b)))
(defun matchid2 (pattern bindings)
  (set-equal (match pattern (sublisl bindings pattern))
	     (cons '(t t) bindings)))
...
(defun matchb (pat inp bindings)
  (and bindings
	   ...
	   (if (varp pat)
		 ...
		 (if binding
		     (and (equal inp (second binding))   ; vs. rest
			  bindings)
		     (cons (list pat inp) bindings)))    ; vs. cons
...
; simple version of sublis that uses (old new) instead of (old . new)
(defun sublisl (alist z)
  (let (pair)
    (if (consp z)
        (cons (sublisl alist (first z))
              (sublisl alist (rest z)))
